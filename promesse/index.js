...
function promessaCasuale() {
    return new Promise((resolve, reject) => {
        ...
        if (valoreCasuale > 0.5) {
            resolve(`Successo! Valore casuale: ${valoreCasuale}`);
        } else {
            reject(`Errore! Valore casuale: ${valoreCasuale}`);
    }
    });
}
...
.then(risultato => {
    ...
    return `${risultato} - Prima manipolazione`;
})
.then(nuovoRisultato => {
    console.log(nuovoRisultato);
    return `${nuovoRisultato} - Seconda manipolazione`;
})
.then(risultatoFinale => {
    console.log("Risultato finale:", risultatoFinale);
})
